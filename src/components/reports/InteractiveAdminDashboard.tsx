...
type ReportPage = "grade" | "class" | "subject" | "student";
type StudentSummary = { id: string; first: ReportRow; rows: ReportRow[]; average: number };

const PAGE_ITEMS = [
  { id: "grade" as const, label: "Toàn khối", description: "Góc nhìn Ban giám hiệu", icon: Building2 },
  { id: "class" as const, label: "Theo lớp", description: "Góc nhìn GVCN", icon: Users },
  { id: "subject" as const, label: "Theo môn", description: "Góc nhìn tổ chuyên môn", icon: BookOpenCheck },
  { id: "student" as const, label: "Cá nhân", description: "Hồ sơ học sinh", icon: UserRound },
];

const PAGE_COPY: Record<ReportPage, { eyebrow: string; title: string; question: string }> = {
  grade: {
    eyebrow: "Dashboard toàn khối",
    title: "Bức tranh chất lượng toàn khối",
    question: "Chất lượng toàn khối đang ở mức nào và xu hướng chung ra sao?",
  },
  class: {
    eyebrow: "Dashboard lớp",
    title: "Hiệu quả học tập của lớp",
    question: "Lớp đang đứng ở đâu so với mặt bằng chung, và em nào cần được kèm cặp?",
  },
  subject: {
    eyebrow: "Dashboard môn",
    title: "Chất lượng theo môn học",
    question: "Đề thi có quá khó không, và phần kiến thức nào học sinh đang còn yếu?",
  },
  student: {
    eyebrow: "Dashboard cá nhân",
    title: "Hồ sơ năng lực học sinh",
    question: "Học sinh đang tiến bộ hay thụt lùi, mạnh và yếu ở môn nào?",
  },
};
...
function SubjectComparison({ studentRows, benchmarkRows, studentLabel }: { studentRows: ReportRow[]; benchmarkRows: ReportRow[]; studentLabel: string }) {
  ...
  })}<div className="flex flex-wrap gap-4 border-t border-border pt-3 text-xs text-muted-foreground"><span><i className="mr-1 inline-block h-2 w-4 rounded bg-blue-600" />{studentLabel}</span><span><i className="mr-1 inline-block h-1.5 w-4 rounded bg-slate-400" />Mức đối chiếu</span></div></div>;
}
...
export default function InteractiveAdminDashboard({ rows }: { rows: ReportRow[] }) {
  ...
  const draftClasses = useMemo(() => unique(rows.filter((row) => !draftGrade || row.khoi === draftGrade).map((row) => row.lop)), [rows, draftGrade]);
  ...
  const changePage = (next: ReportPage) => {
    setPage(next);
    if (next !== "class") setClassName("");
    if (next !== "subject") setSubject("");
  ...
  return <div className="overflow-hidden rounded-2xl border border-border bg-muted/20 shadow-sm">
    <header className="border-b border-border bg-card px-5 py-5 sm:px-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div><p className="flex items-center gap-2 text-sm font-semibold text-blue-700 dark:text-blue-300"><LayoutDashboard className="h-4 w-4" />Trung tâm báo cáo</p><h1 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl">Dashboard kết quả thi</h1><p className="mt-1 text-sm text-muted-foreground">Một module BI · bốn góc nhìn quản trị · cùng một nguồn dữ liệu</p></div>
    ...
    <nav aria-label="Các trang báo cáo" className="grid border-b border-border bg-card sm:grid-cols-2 xl:grid-cols-4">{PAGE_ITEMS.map((item) => {
    ...
    <main className="min-w-0 space-y-5 p-4 sm:p-6">
      ...
      {page === "class" && <ClassDashboard rows={baseRows} className={selectedClass} />}
      {page === "subject" && <SubjectDashboard rows={baseRows} subject={selectedSubject} />}
    ...
    {filterOpen && <div className="fixed inset-0 z-50" role="presentation">
      ...
      <aside role="dialog" aria-modal="true" aria-labelledby="filter-title" className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col border-l border-border bg-card shadow-2xl animate-in slide-in-from-right duration-200 motion-reduce:animate-none">
        <header className="flex items-center justify-between border-b border-border px-5 py-4"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-blue-700 dark:text-blue-300">Thiết lập phạm vi</p><h2 id="filter-title" className="mt-1 text-xl font-bold">Bộ lọc báo cáo</h2></div><button ref={closeFilterButtonRef} onClick={() => setFilterOpen(false)} aria-label="Đóng" className="flex h-11 w-11 cursor-pointer items-center justify-center rounded-lg transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><X className="h-5 w-5" /></button></header>
        <div className="flex-1 space-y-5 overflow-y-auto p-5">
          ...
          {page === "class" && <FilterSelect label="Lớp" value={draftClassName} onChange={setDraftClassName} options={draftClasses} allLabel="Chọn lớp" required />}
          {page === "subject" && <FilterSelect label="Môn học" value={draftSubject} onChange={setDraftSubject} options={subjects} allLabel="Chọn môn" required />}
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-xs leading-5 text-blue-900 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-200"><strong>Phạm vi sau khi áp dụng</strong><br />{draftBatch || "Tất cả đợt thi"} · {draftGrade ? `Khối ${draftGrade}` : "Toàn trường"}{draftClassName ? ` · ${draftClassName}` : ""}{draftSubject ? ` · ${draftSubject}` : ""}</div>
        </div>
...
function GradeDashboard({ rows }: { rows: ReportRow[] }) {
  ...
  return <><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><KpiCard label="Học sinh tham gia" value={totalStudents.toLocaleString("vi-VN")} detail={`${rows.length} lượt bài thi`} icon={Users} /><KpiCard label="Điểm trung bình" value={average(rows.map((row) => row.diem)).toFixed(2)} detail="Trên thang điểm 10" icon={TrendingUp} tone="green" /><KpiCard label="Tỷ lệ đạt" value={`${passRate.toFixed(1)}%`} detail="Điểm từ 5.0 trở lên" icon={CheckCircle2} tone="blue" /><KpiCard label="Tỷ lệ dưới 5" value={`${weakRate.toFixed(1)}%`} detail="Cần theo dõi chất lượng" icon={AlertTriangle} tone="red" /></section>
    <section className="grid gap-5 xl:grid-cols-2"><Panel title="Xu hướng điểm trung bình" subtitle="So sánh kết quả qua các đợt thi"><TrendChart rows={rows} /></Panel><Panel title="Xếp hạng lớp" subtitle="Điểm trung bình từ cao xuống thấp"><HorizontalBars items={classStats} /></Panel></section>
    {weakRate >= 20 && <div className="flex gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><div><strong>Cảnh báo chất lượng</strong><p className="mt-1">Tỷ lệ bài thi dưới trung bình đang ở mức {weakRate.toFixed(1)}%. Cần rà soát các lớp và môn có kết quả thấp.</p></div></div>}
    <Panel title="Top 10 học sinh toàn khối" subtitle="Xếp hạng theo điểm trung bình trong phạm vi lọc"><div className="overflow-x-auto"><table className="w-full min-w-[620px] text-sm"><thead><tr className="border-b border-border text-left text-xs uppercase tracking-wide text-muted-foreground"><th className="pb-3">Hạng</th><th className="pb-3">Học sinh</th><th className="pb-3">Lớp</th><th className="pb-3 text-right">Số bài</th><th className="pb-3 text-right">Điểm TB</th></tr></thead><tbody>{topStudents.map((student, index) => <tr key={student.first.hocSinhId} className="border-b border-border/60 last:border-0"><td className="py-3 font-bold text-blue-700">#{index + 1}</td><td className="py-3"><strong>{student.first.hoTen}</strong><span className="ml-2 text-xs text-muted-foreground">{student.first.maSo}</span></td><td className="py-3">{student.first.lop}</td><td className="py-3 text-right tabular-nums">{student.count}</td><td className="py-3 text-right text-base font-bold tabular-nums">{student.average.toFixed(2)}</td></tr>)}</tbody></table></div></Panel></>;
}

function ClassDashboard({ rows, className }: { rows: ReportRow[]; className: string }) {
  const classRows = rows.filter((row) => row.lop === className);
  const classStudents = new Map<string, ReportRow[]>(); classRows.forEach((row) => classStudents.set(row.hocSinhId, [...(classStudents.get(row.hocSinhId) || []), row]));
  const students = [...classStudents.values()].map((list) => ({ first: list[0]!, average: average(list.map((row) => row.diem)), rows: list })).sort((a, b) => b.average - a.average);
  const rankings = unique(rows.map((row) => row.lop)).map((name) => ({ name, value: average(rows.filter((row) => row.lop === name).map((row) => row.diem)) })).sort((a, b) => b.value - a.value);
  const rank = Math.max(1, rankings.findIndex((item) => item.name === className) + 1);
  const weak = students.filter((student) => student.average < 5);
  const excellent = students.filter((student) => student.average >= 8);
  const subjectStats = unique(rows.map((row) => row.mon)).map((name) => ({ label: name, own: average(classRows.filter((row) => row.mon === name).map((row) => row.diem)), all: average(rows.filter((row) => row.mon === name).map((row) => row.diem)) }));
  if (!className) return <EmptySelection text="Chọn một lớp ở bộ lọc để mở dashboard lớp." />;
  return <><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><KpiCard label="Điểm TB lớp" value={average(classRows.map((row) => row.diem)).toFixed(2)} detail={`Mặt bằng khối ${average(rows.map((row) => row.diem)).toFixed(2)}`} icon={TrendingUp} tone="blue" /><KpiCard label="Xếp hạng" value={`${rank}/${rankings.length}`} detail="Trong phạm vi khối đã chọn" icon={BarChart3} tone="amber" /><KpiCard label="Học sinh giỏi" value={String(excellent.length)} detail="Điểm trung bình từ 8.0" icon={GraduationCap} tone="green" /><KpiCard label="Cần quan tâm" value={String(weak.length)} detail="Điểm trung bình dưới 5.0" icon={AlertTriangle} tone="red" /></section>
    <section className="grid gap-5 xl:grid-cols-2"><Panel title={`Phổ điểm lớp ${className}`} subtitle="Phân bố lượt bài thi theo bốn khoảng điểm"><ScoreDistribution rows={classRows} /></Panel><Panel title="Năng lực theo môn" subtitle="Lớp (xanh) so với mặt bằng khối (xám)"><div className="space-y-4">{subjectStats.map((item) => <div key={item.label}><div className="mb-2 flex justify-between text-sm"><strong>{item.label}</strong><span><b className="text-blue-600">{item.own.toFixed(2)}</b> / <span className="text-muted-foreground">{item.all.toFixed(2)}</span></span></div><div className="space-y-1"><div className="h-2.5 rounded-full bg-muted"><div className="h-full rounded-full bg-blue-600" style={{ width: `${item.own * 10}%` }} /></div><div className="h-1.5 rounded-full bg-muted"><div className="h-full rounded-full bg-slate-400" style={{ width: `${item.all * 10}%` }} /></div></div></div>)}</div></Panel></section>
    <Panel title="Danh sách học sinh" subtitle="Ưu tiên các trường hợp cần can thiệp"><div className="overflow-x-auto"><table className="w-full min-w-[640px] text-sm"><thead><tr className="border-b text-left text-xs uppercase text-muted-foreground"><th className="pb-3">Học sinh</th><th className="pb-3">Mã số</th><th className="pb-3 text-right">Điểm TB</th><th className="pb-3">Trạng thái</th></tr></thead><tbody>{students.map((student) => <tr key={student.first.hocSinhId} className="border-b border-border/60 last:border-0"><td className="py-3 font-semibold">{student.first.hoTen}</td><td className="py-3 text-muted-foreground">{student.first.maSo}</td><td className="py-3 text-right font-bold">{student.average.toFixed(2)}</td><td className="py-3">{student.average < 5 ? <span className="rounded-full bg-rose-100 px-2.5 py-1 text-xs font-semibold text-rose-700 dark:bg-rose-950 dark:text-rose-300">Cần quan tâm</span> : <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">Ổn định</span>}</td></tr>)}</tbody></table></div></Panel></>;
}
...
function SubjectDashboard({ rows, subject }: { rows: ReportRow[]; subject: string }) {
  ...
  if (!subject) return <EmptySelection text="Chọn một môn học ở bộ lọc để mở dashboard chuyên môn." />;
  return <><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><KpiCard label="Điểm trung bình" value={average(scores).toFixed(2)} detail={`${subjectRows.length} lượt bài thi`} icon={TrendingUp} tone="blue" /><KpiCard label="Điểm cao nhất" value={(Math.max(0, ...scores)).toFixed(2)} detail="Kết quả nổi bật" icon={GraduationCap} tone="green" /><KpiCard label="Điểm thấp nhất" value={(scores.length ? Math.min(...scores) : 0).toFixed(2)} detail="Mức cần lưu ý" icon={TrendingDown} tone="red" /><KpiCard label="Số điểm 10" value={String(scores.filter((score) => score === 10).length)} detail="Điểm tuyệt đối" icon={CheckCircle2} tone="amber" /></section>
    <section className="grid gap-5 xl:grid-cols-2"><Panel title={`Phổ điểm môn ${subject}`} subtitle="Quan sát độ lệch của phân phối điểm"><ScoreDistribution rows={subjectRows} /></Panel><Panel title="So sánh các lớp" subtitle="Xếp hạng điểm trung bình môn"><HorizontalBars items={classes} /></Panel></section>
    <Panel title="Chỉ báo chuyên môn" subtitle="Tổng hợp từ dữ liệu kết quả hiện có"><div className="grid gap-4 sm:grid-cols-3"><Insight label="Tỷ lệ dưới trung bình" value={`${percent(scores.filter((score) => score < 5).length, scores.length).toFixed(1)}%`} note="Dùng để nhận diện đề khó hoặc lỗ hổng kiến thức." tone="red" /><Insight label="Tỷ lệ khá, giỏi" value={`${percent(scores.filter((score) => score >= 6.5).length, scores.length).toFixed(1)}%`} note="Điểm từ 6.5 trở lên." tone="green" /><Insight label="Độ chênh lớp" value={`${classes.length ? (classes[0]!.value - classes.at(-1)!.value).toFixed(2) : "0.00"} điểm`} note="Khoảng cách giữa lớp cao nhất và thấp nhất." tone="amber" /></div></Panel></>;
}
...
function StudentDashboard({ rows, student, students, search, onSearchChange, onSelect, onBack }: {
  ...
  if (!student) {
    ...
    return <Panel title="Danh sách học sinh" subtitle={`${visibleStudents.length} học sinh trong phạm vi báo cáo`}>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="text-sm text-muted-foreground">Bấm vào một dòng để xem hồ sơ năng lực</p>
      </div>
  ...
  return <><button onClick={onBack} className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border border-border bg-card px-3 text-sm font-semibold transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ArrowLeft className="h-4 w-4" />Quay lại danh sách học sinh</button><section className="flex flex-wrap items-center gap-4 rounded-xl border border-border bg-card p-5 shadow-sm"><span className="flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 text-xl font-bold text-blue-700 dark:bg-blue-950 dark:text-blue-300">{student.first.hoTen.trim().charAt(0).toUpperCase()}</span><div><h3 className="text-xl font-bold">{student.first.hoTen}</h3><p className="text-sm text-muted-foreground">{student.first.maSo} · Lớp {student.first.lop}</p></div><span className="ml-auto rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">Đang theo học</span></section>
    <section className="grid gap-3 sm:grid-cols-3"><KpiCard label="Điểm TB cá nhân" value={student.average.toFixed(2)} detail={`${studentRows.length} lượt bài thi`} icon={TrendingUp} tone="blue" /><KpiCard label="Xếp hạng lớp" value={`${rankClass}/${classStudents.length}`} detail={`Trong lớp ${student.first.lop}`} icon={Users} tone="green" /><KpiCard label="Xếp hạng khối" value={`${rankAll}/${rankedAll.length}`} detail="Trong phạm vi đang lọc" icon={Building2} tone="amber" /></section>
    <section className="grid gap-5 xl:grid-cols-2"><Panel title="Tiến độ qua các đợt thi" subtitle="Theo dõi xu hướng điểm trung bình cá nhân"><TrendChart rows={studentRows} highlight={student.first.hoTen} /></Panel><Panel title="Năng lực các môn" subtitle="Cá nhân (xanh) so với trung bình lớp (xám)"><SubjectComparison studentRows={studentRows} benchmarkRows={rows.filter((row) => row.lop === student.first.lop)} studentLabel="Cá nhân" /></Panel></section>
    {weakest && <div className="flex gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-200"><GraduationCap className="mt-0.5 h-5 w-5 shrink-0" /><div><strong>Đề xuất học tập</strong><p className="mt-1">Nên ưu tiên cải thiện môn {weakest.name} (điểm trung bình {weakest.value.toFixed(2)}) để nâng kết quả tổng thể và thứ hạng.</p></div></div>}
    <Panel title="Chi tiết điểm hiện tại" subtitle="Trạng thái đạt được xác định theo ngưỡng 5.0"><div className="overflow-x-auto"><table className="w-full min-w-[600px] text-sm"><thead><tr className="border-b text-left text-xs uppercase text-muted-foreground"><th className="pb-3">Môn học</th><th className="pb-3">Đợt thi</th><th className="pb-3 text-right">Điểm</th><th className="pb-3">Trạng thái</th></tr></thead><tbody>{studentRows.map((row) => <tr key={row.baiLamId} className="border-b border-border/60 last:border-0"><td className="py-3 font-semibold">{row.mon}</td><td className="py-3 text-muted-foreground">{row.dotThi}</td><td className="py-3 text-right text-base font-bold">{row.diem.toFixed(2)}</td><td className="py-3">{row.diem >= 5 ? <span className="text-emerald-700 dark:text-emerald-300">Đạt</span> : <span className="text-rose-700 dark:text-rose-300">Chưa đạt</span>}</td></tr>)}</tbody></table></div></Panel></>;
}
