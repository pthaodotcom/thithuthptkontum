...
export default async function TaoDeThiPage(){
 ...
 const s=taoSupabaseServiceRole();const {data:mon}=await s.from("mon").select("*").eq("to_truong_tai_khoan_id",session.sub).maybeSingle();if(!mon)redirect("/soan-cau-hoi");
 const [{data:cd},{data:md},{data:ca},{data:de},{data:cauHoi}]=await Promise.all([
  ...
  s.from("de_thi").select("de_thi_id,trang_thai,so_ma_de,created_at,ca_thi_mon!inner(mon_id,ca_thi(so_thu_tu_ca,dot_thi(ten_dot:ten_dot_thi)))").eq("ca_thi_mon.mon_id",mon.mon_id).order("created_at",{ascending:false}),
