...
export async function xuLyNgoaiLe(input:z.input<typeof schema>){
 ...
 if(error){const map:Record<string,string>={CA_THI_KHONG_DANG_MO:"Ca thi không còn mở.",TRANG_THAI_DA_THAY_DOI:"Bài thi vừa được cập nhật. Vui lòng tải lại danh sách.",VUOT_QUA_HAI_LAN_RESET:"Học sinh đã dùng đủ 2 lần làm lại bài."};const k=Object.keys(map).find(x=>error.message.includes(x));return{success:false,error:k?map[k]:"Chưa xử lý được yêu cầu. Vui lòng thử lại."}}
