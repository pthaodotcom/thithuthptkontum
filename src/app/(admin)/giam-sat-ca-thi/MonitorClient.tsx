  ...
  return (
    <div className="space-y-5">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Xem các lần học sinh sao chép nội dung, rời màn hình hoặc mất kết nối trong khi làm bài.</p>
      </header>
      ...
      <div className="flex flex-wrap gap-2">
        <span className="rounded-full bg-muted px-3 py-1 text-sm font-medium">Tổng số lần vi phạm: {summary.total}</span>
        <span className="rounded-full bg-muted px-3 py-1 text-sm font-medium">Học sinh có vi phạm: {summary.students}</span>
      </div>

      <div className="overflow-auto rounded-xl border border-border bg-card">
        <Table>
          <TableHeader><TableRow className="bg-muted/50 hover:bg-muted/50"><TableHead>Học sinh</TableHead><TableHead>Đợt thi</TableHead><TableHead>Môn</TableHead><TableHead>Loại vi phạm gần nhất</TableHead><TableHead className="text-center">Số lần</TableHead><TableHead>Thời điểm gần nhất</TableHead><TableHead className="text-right">Thao tác</TableHead></TableRow></TableHeader>
          <TableBody>
            {data.length === 0 && <TableRow><TableCell colSpan={7} className="py-10 text-center text-muted-foreground">Chưa ghi nhận vi phạm nào.</TableCell></TableRow>}
            {data.map((item) => {
              ...
              return <TableRow key={item.bai_lam_id}>
                ...
                <TableCell>{latest ? (NHAN_LOAI[latest.loai_vi_pham] ?? "Vi phạm khác") : "—"}</TableCell>
                <TableCell className="text-center"><span className="font-semibold text-destructive">{violations.length}</span></TableCell>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><ShieldAlert className="h-5 w-5 text-destructive" />Chi tiết các lần vi phạm</DialogTitle></DialogHeader>
          {selected && <div className="space-y-3">
            <p className="text-sm">Học sinh: <b>{(Array.isArray(selected.tai_khoan) ? selected.tai_khoan[0] : selected.tai_khoan)?.ho_ten}</b> ({((Array.isArray(selected.tai_khoan) ? selected.tai_khoan[0] : selected.tai_khoan)?.ma_so)})</p>
            <div className="max-h-80 space-y-2 overflow-auto">
              {[...(selected.vi_pham ?? [])].sort((a,b) => Date.parse(b.thoi_diem)-Date.parse(a.thoi_diem)).map((viPham, index) => <div key={viPham.id} className="flex items-center justify-between rounded-lg border p-3 text-sm"><span><b>Lần {selected.vi_pham!.length - index}:</b> {NHAN_LOAI[viPham.loai_vi_pham] ?? "Vi phạm khác"}</span><time className="text-muted-foreground">{new Date(viPham.thoi_diem).toLocaleString("vi-VN")}</time></div>)}
            </div>
          </div>}
        </DialogContent>
      </Dialog>
