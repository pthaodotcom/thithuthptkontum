...
export async function boNhiemToTruong(input:z.input<typeof schema>){
 ...
 if(error){if(error.message.includes("GIAO_VIEN_KHONG_THUOC_MON"))return{success:false,error:"Giáo viên không phụ trách môn này hoặc tài khoản đang tạm khóa."};if(error.message.includes("GIAO_VIEN_DANG_LA_TO_TRUONG_MON_KHAC"))return{success:false,error:"Giáo viên đang là Tổ trưởng của môn khác."};return{success:false,error:"Chưa bổ nhiệm được Tổ trưởng. Vui lòng thử lại."}}
