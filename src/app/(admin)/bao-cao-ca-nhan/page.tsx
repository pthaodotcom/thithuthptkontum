...
const nhanViPham: Record<string, string> = { Copy: "Sao chép nội dung", ChuyenTab: "Chuyển tab/ứng dụng", MatKetNoi: "Mất kết nối" };
const nhanNguonNhanXet: Record<string, string> = { AI: "Tạo tự động", Fallback: "Theo mẫu có sẵn" };
...
export default async function BaoCaoCaNhanAdminPage({ searchParams }: { searchParams: Promise<{ q?: string; hocSinhId?: string }> }) {
  ...
  if (!hocSinhId) {
    ...
    return <div className="space-y-6"><div><p className="flex items-center gap-2 text-sm font-semibold text-accent"><GraduationCap className="h-4 w-4" />Kết quả học sinh</p><h1 className="mt-2 text-3xl font-bold">Báo cáo cá nhân</h1><p className="mt-1 text-muted-foreground">Chọn một học sinh để xem điểm và kết quả từng lần thi.</p></div>
  ...
  return <div className="space-y-6"><Link href="/bao-cao-ca-nhan" className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><ArrowLeft className="h-4 w-4" />Danh sách học sinh</Link><div><p className="text-sm font-semibold text-accent">Kết quả cá nhân</p><h1 className="mt-2 text-3xl font-bold">{first.hoTen}</h1><p className="text-muted-foreground">{first.maSo} · {first.lop}</p></div>
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{[
      { label: "Số bài đã thi", value: String(rows.length), icon: BarChart3 }, { label: "Điểm trung bình", value: average.toFixed(2), icon: TrendingUp }, { label: "Điểm cao nhất", value: best.toFixed(2), icon: Trophy }, { label: "Tổng vi phạm", value: String(rows.reduce((sum, row) => sum + row.soViPham, 0)), icon: BarChart3 }, { label: "Nhóm hiện tại", value: first.nhom === "DaNamVung" ? "Nắm vững" : first.nhom === "Kha" ? "Khá" : first.nhom === "TrungBinh" ? "Trung bình" : "Cần ôn tập", icon: Sparkles },
    ...
    <section className="space-y-4"><h2 className="text-xl font-bold">Kết quả từng lần thi</h2>{rows.map((row) => { const comment = commentMap.get(row.baiLamId); const noiDungNhanXet = lamSachNhanXet(comment?.noi_dung || ""); return <article key={row.baiLamId} className="rounded-2xl border border-border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="font-bold">{row.mon} · {row.dotThi}</h3><p className="text-sm text-muted-foreground">Đúng {row.soCauDung} · Sai {row.soCauSai}</p></div><p className="text-3xl font-bold tabular-nums">{row.diem.toFixed(2)}</p></div>{row.soViPham > 0 && <div className="mt-4 rounded-xl border border-destructive/20 bg-destructive/5 p-4"><p className="font-semibold text-destructive">Vi phạm: {row.soViPham} lần{row.tuDongThuBaiDoViPham ? " · Bài đã bị thu do vi phạm" : ""}</p><p className="mt-1 text-sm text-muted-foreground">{row.loaiViPham.map((loai) => nhanViPham[loai] ?? "Vi phạm khác").join(" · ")}{row.viPhamGanNhat ? ` · Gần nhất ${new Date(row.viPhamGanNhat).toLocaleString("vi-VN")}` : ""}</p></div>}<div className="mt-4 rounded-xl bg-muted p-4"><div className="flex items-center gap-2"><span className="rounded-full bg-card px-2 py-1 text-xs font-semibold">{comment?.nguon ? (nhanNguonNhanXet[comment.nguon] ?? "Đã tạo") : "Chưa có"}</span><span className="text-xs text-muted-foreground">Nhận xét</span></div><p className="mt-2 whitespace-pre-wrap leading-7 text-foreground">{noiDungNhanXet || "Chưa có nhận xét."}</p></div><div className="mt-4 flex justify-end"><Link href={`/tra-cuu?baiLamId=${row.baiLamId}`} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-border px-3 text-sm font-semibold hover:bg-muted">Xem chi tiết bài thi <ArrowRight className="h-4 w-4" /></Link></div></article>; })}</section>
  </div>;
}
