...
export default async function DoiMonTuChonPage(){
 ...
 const dots=(phamVi??[]).map((x:any)=>{const d=Array.isArray(x.dot_thi)?x.dot_thi[0]:x.dot_thi;const ca=(d?.ca_thi??[]).find((c:any)=>c.so_thu_tu_ca===3);return{dot_thi_id:x.dot_thi_id,ten_dot_thi:d?.ten_dot_thi??"",han_doi:new Date(new Date(ca?.gio_bat_dau).getTime()-48*3600_000).toISOString(),so_lan_tc1:(logs??[]).filter(l=>l.dot_thi_id===x.dot_thi_id&&l.vi_tri==="TC1").length,so_lan_tc2:(logs??[]).filter(l=>l.dot_thi_id===x.dot_thi_id&&l.vi_tri==="TC2").length}}).filter(x=>x.han_doi);
